...
import { SwitchAccountButton } from '@/components/molecules/buttons/SwitchAccountButton';
import { SignoutButton } from '@/components/molecules/buttons/SignoutButton';
interface Props {
    active: boolean;
}
interface States {
            ...
            <ul className={ `${ active ? 'block' : 'hidden' } shadow-lg absolute top-full right-0 py-5 min-w-full bg-white min-w-max` } onClick={ stopPropagation<HTMLUListElement>(  ) }>
                ...
                <li className={ `select-none` }>
                    ...
                    <div className={ `${ switchAccount.formArea.active ? 'py-2' : 'h-0' } overflow-y-hidden transition duration-150 px-2 text-right` }>
                        ...
                        {/* サインインユーザー切り替え送信 */}
                        <SwitchAccountButton
                            getParams={ (  ) => ( { email: ( this.email.current?.value || '' ), password: ( this.password.current?.value || '' ) } ) }
