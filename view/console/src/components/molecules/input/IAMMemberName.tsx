...
export const IAMMemberName: React.ForwardRefExoticComponent<React.RefAttributes<Handler>> = React.forwardRef<Handler, Props>( ( props, ref ) => {
    ...
    return (
        <label className='inline-block w-full cursor-pointer my-3 text-gray-600'>
            ...
            <Text ref={ ref } name='name' autoFocus={ true } required placeholder='Member account Name' minLength={ 4 } maxLength={ 100 } className='text-base text-gray-500 font-bold w-full cursor-pointer outline-none focus:outline-none focus:ring-0 border-t-0 border-r-0 border-l-0 border-gray-500 focus:border-blue-500 bg-transparent py-1' onValidate={ onValidate } />
        </label>
    );
} );
